...
var dependencies =
    '<!-- light-modal -->'
    + '<link href="dist/light-modal.css" rel="stylesheet">'
    + '<!-- animate.css -->'
    + '<link href="dist/animate.css" rel="stylesheet">'
    + '<!-- canvas-toBlob.js -->'
    + '<script src="dist/canvas-toBlob.js" defer></script>'
    + '<!-- FileSaver.js -->'
    + '<script src="dist/FileSaver.js" defer></script>'
    + '<!-- Croppr.js -->'
    + '<link href="dist/croppr.css" rel="stylesheet">'
  ...
  cropzee: function (options = {
        ...
        window.cropzeeCreateImage = function(id) {
            ...
            if (options.returnImageMode == 'blob') {
				...
				salvarImagem(cropzeeReturnImage[id]);
            }
            ...
            '<div class="light-modal" id="cropzee-modal" role="dialog" aria-labelledby="light-modal-label" aria-hidden="false" data-lightmodal="close">'
                + '<div class="light-modal-content ' + animation + '">'
                    ...
                    + '<div class="light-modal-footer" style="justify-content: space-between;">'
                        + '<div onclick="closeModal()" class="light-modal-close-btn" style="cursor: pointer;" aria-label="close">Cancelar</div>'
                        + '<div onclick="cropzeeRotateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Girar 90deg</div>'
                        + '<div onclick="cropzeeCreateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Cortar</div>'
                    + '</div>'
                + '</div>'
...
function closeModal() {
	...
	window.location=data1.replace("#cropzee-modal", "")+ '#';
}